...
export async function upload_screenshot(req,res,next){try{const{session_id,file_key,file_url,width,height,monitor_index,captured_at}=req.body;if(!file_key)return fail(res,'file_key required');const data={user_id:req.user.id,file_key,file_url,width:width?+width:null,height:height?+height:null,monitor_index:monitor_index?+monitor_index:0,captured_at:captured_at?new Date(captured_at):new Date()};if(session_id)data.session_id=session_id;const s=await prisma.screenshots.create({data});return ok(res,s,'Screenshot recorded',201);}catch(e){next(e);}}
