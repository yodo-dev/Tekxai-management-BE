...
// Templates
export async function list_templates(req,res,next){try{const t=await prisma.contract_templates.findMany({
  take: 500,where:{is_active:true},orderBy:{name:'asc'}});return ok(res,t);}catch(e){next(e);}}
...
// Contracts
export async function list_contracts(req,res,next){try{const is_admin=req.user.roles.some(r=>['ADMIN','SUPER_ADMIN','HR'].includes(r));const where=is_admin?(req.query.user_id?{user_id:req.query.user_id}:{}):{user_id:req.user.id};const contracts=await prisma.contracts.findMany({
  take: 500,where,orderBy:{created_at:'desc'},include:{user:{select:{id:true,first_name:true,last_name:true}},template:{select:{name:true}}}});return ok(res,{records:contracts,total:contracts.length});}catch(e){next(e);}}
