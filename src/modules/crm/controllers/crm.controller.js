...
export async function list_clients(req,res,next){try{const clients=await prisma.client_accounts.findMany({orderBy:{name:'asc'},include:{project_access:{select:{project_id:true}}}});return ok(res,{records:clients,total:clients.length});}catch(e){next(e);}}
...
export async function list_client_projects(req,res,next){try{const accesses=await prisma.client_project_access.findMany({where:{client_id:req.params.id},include:{project:{select:{id:true,title:true,status:true,progress:true,end_date:true}}}});return ok(res,accesses);}catch(e){next(e);}}
